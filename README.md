...
Самое большое количество rps требуется для поиска (OpenSearch, fallback до PostgreSQL в случае отсутствия данных в OpenSearch), поэтому можно разместить серверы поисковых движков в указанных выше регионах для увеличения скорости выдачи результатов. 
